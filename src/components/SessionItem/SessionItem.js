                    ...
                    <div className="media-body">
                        <h4 className="media-heading">
                            <Link
                                ...
                                className={(new Date(session.closedAt).getTime() - Date.now() > 0 || !session.closedAt) ? "session-title" : "session-title-p"}
                            >Chủ đề: {session.topic} {(new Date(session.closedAt).getTime() - Date.now() > 0 || !session.closedAt) ? "" : "(Đã đóng)"}
                        ...
                        <p className="session-number"><span>{session.questions.length === 0 ? "Chưa có câu hỏi" : `Số câu hỏi: ${session.questions.length}`} </span> - Tạo lúc: {(new Date(session.createdAt)).toLocaleString()} - Đóng lúc: {!session.closedAt ? "Chưa xác định" : (new Date(session.closedAt)).toLocaleString()}</p>
